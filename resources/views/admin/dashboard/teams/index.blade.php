...
@section('title_page', trans('teams.show_teams'))
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body  search-group">
                    <div class="row">
                        <div class="col-md-12 text-end mb-2">
                            <a href="{{ route('admin.teams.create') }}" class="btn btn-outline-success btn-sm">@lang('admin.create')</a>
                        </div>
                    </div>
                    {{-- Start Form search --}}
                    <form action="{{ route('admin.teams.index') }}" method="get">
                        <div class="row mb-3">
                            ...
                            <div class="col-md-3 mb-2">
                                <select class="form-select" name="is_directors" aria-label=".form-select-sm example">
                                    <option selected value=""> @lang('admin.is_directors') </option>
                                    <option value="1" {{ old('is_directors', request()->input('is_directors')) == 1 ? 'selected' : '' }}>
                                        @lang('admin.yes') </option>
                                    <option value="0" {{ old('is_directors', request()->input('is_directors')) != 1 && old('is_directors', request()->input('is_directors')) != null ? 'selected' : '' }}>
                            ...
                            <div class="search-input col-md-2">
                                ...
                                <a class="btn btn-success btn-sm" href="{{ route('admin.teams.index') }}"><i class="refresh ion ion-md-refresh"></i></a>
                            </div>
                        </div>
                    </form>
                    {{-- End Form search --}}
                ...
                <div class="card-body mt-0 pt-0">
                    <form id="update-pages" action="{{ route('admin.teams.actions') }}" method="post">
                        @csrf
                    </form>
                    <div class="table-responsive">
                        <table id="main-datatable" class="table table-bordered  dt-responsive nowrap table-striped table-table-success table-hover" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center">
                                            @if ($item->status == 1)
                                            <a href="{{ route('admin.teams.update-status', $item->id) }}" title="@lang('admin.active')" class="btn btn-xs btn-success btn-sm m-1"><i class="fa fa-check"></i></a>
                                            @else
                                            <a href="{{ route('admin.teams.update-status', $item->id) }}" title="@lang('admin.dis_active')" class="btn btn-xs btn-outline-secondary btn-sm m-1"><i class="fa fa-ban"></i></a>
                                            @endif

                                            @if ($item->feature == 1)
                                            <a href="{{ route('admin.teams.update-featured', $item->id) }}" title="@lang('admin.feature')" class="btn btn-xs btn-warning btn-sm m-1"><i class="fa fa-star"></i></a>
                                            @else
                                            <a href="{{ route('admin.teams.update-featured', $item->id) }}" title="@lang('admin.feature')" class="btn btn-xs btn-outline-secondary btn-sm m-1"><i class="fa fa-star"></i></a>
                                            @endif

                                            @if ($item->is_directors == 1)
                                                <a href="{{ route('admin.teams.update-directors', $item->id) }}" title="@lang('admin.directors')" class="btn btn-xs btn-warning btn-sm m-1"><i class="fas fa-users"></i></a>
                                            @else
                                                <a href="{{ route('admin.teams.update-directors', $item->id) }}" title="@lang('admin.directors')" class="btn btn-xs btn-outline-secondary btn-sm m-1"><i class="fas fa-users"></i></a>
                                            @endif



                                            <a href="{{ route('admin.teams.show', $item->id) }}" title="@lang('admin.show')" class="btn btn-xs btn-outline-info btn-sm m-1"><i class="fas fa-eye"></i></a>


                                            <a href="{{ route('admin.teams.edit', $item->id) }}" title="@lang('admin.edit')" class="btn btn-outline-primary btn-sm m-1"><i class="fas fa-pencil-alt"></i></a>
                                ...
                                @include('admin.dashboard.teams.delete')
                                @endforeach
