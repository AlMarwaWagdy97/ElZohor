...
class MenueController extends Controller
{
    ...
    public function getUrl(Request $request){
        ...
        if($name == UrlTypesEnum::SPECIALTIES){
            $items = Specialties::query()->with('trans')->active()->get(['id']);
            foreach($items as $item){
                $res[] =  '/specialities/' . $item->trans->where("locale", app()->getLocale())->first()->slug;
            }
        }

        if($name == UrlTypesEnum::DOCTORS){
            $items = Doctor::query()->with('trans')->active()->get(['id']);
            foreach($items as $item){
                $res[] =  '/doctors/' . $item->trans->where("locale", app()->getLocale())->first()->slug;
            }
        }
        ...
        if($name == UrlTypesEnum::GALLERY){
            $items = Gallery::query()->with('trans')->active()->get(['id']);
            foreach($items as $item){
                $res[] =  '/gallery/' . $item->trans->where("locale", app()->getLocale())->first()->slug;
            }
        }
