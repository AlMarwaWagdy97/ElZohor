...
class MenueController extends Controller
{
    ...
    public function getUrl(Request $request)
    {
        ...
        if ($name == UrlTypesEnum::CLIENTS) {
            $items = Client::query()->with('trans')->active()->get(['id']);
            foreach ($items as $item) {
                $res[] =  '/clients/' . $item->trans->where("locale", app()->getLocale())->first()->slug;
            }
        }


        if ($name == UrlTypesEnum::TEAMS) {
            $items = Teams::query()->with('trans')->active()->get(['id']);
            foreach ($items as $item) {
                $res[] =  '/teams/' . $item->trans->where("locale", app()->getLocale())->first()->slug;
            }
        }

        if ($name == UrlTypesEnum::VIDEO) {
            $items = Video::query()->with('trans')->active()->get(['id']);
            foreach ($items as $item) {
                $res[] =  '/videos/' . $item->trans->where("locale", app()->getLocale())->first()->slug;
            }
        }

        if ($name == UrlTypesEnum::CERTIFICATIONS) {
            $items = Gallery::query()->with('trans')->active()->get(['id']);
            foreach ($items as $item) {
                $res[] =  '/certifications/' . $item->trans->where("locale", app()->getLocale())->first()->slug;
            }
        }


        if ($name == UrlTypesEnum::CAREERS) {
            $items = Video::query()->with('trans')->active()->get(['id']);   //here
            foreach ($items as $item) {
                $res[] =  '/careers/' . $item->trans->where("locale", app()->getLocale())->first()->slug;
            }
        }
        ...
        if ($name == UrlTypesEnum::NEWS) {                                              //here
            $items = Video::query()->with('trans')->active()->get(['id']);
            foreach ($items as $item) {
